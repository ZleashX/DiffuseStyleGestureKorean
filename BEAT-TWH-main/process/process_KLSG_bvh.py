...
bone_names = ['Spine', 'Spine1', 'Neck', 'Head',
...
def load_audio(audiofile, wavlm_model, cfg, device=torch.device('cuda:0')):
    ...
    onsets_f = librosa.onset.onset_detect(y=wav, sr=sr, units='time')
...
def make_gesture_dataset(base_path, save_path, preload=False, wavlm_model=None, cfg=None, word2vector=None,
    ...
    motion_save_path = os.path.join(save_path, 'gesture_KLSG')
    audio_save_path = os.path.join(save_path, 'audio_KLSG')
    text_save_path = os.path.join(save_path, 'text_KLSG')
    ...
    if preload:
        with h5py.File(f"KLSG_" + version + ".h5", "w") as h5:
            total_index = 0
            for speaker in os.listdir(base_path):
                print('Processing speaker {}'.format(speaker))
                gesture_path = os.path.join(base_path, speaker)
                ...
                for v_i, bvh_file in enumerate(bvh_files):
                    ...
                    wav = wav[:clip_len]
                    tsv = tsv[:clip_len]
                    g_data.create_dataset("speaker_id", data=[int(speaker)])  # TODO style
...
def main():
    ...
    parser.add_argument("--db_path", type=Path, default="../../KLSG_dataset/source/")
    parser.add_argument("--save_path", type=Path, default="../../KLSG_dataset/processed/")
    parser.add_argument('--wavlm_path', type=str, default='./WavLM/WavLM-Large.pt')
    parser.add_argument('--word2vector_path', type=str, default='./cc.ko.300.vec')
    parser.add_argument("version", type=str, default="v0")
    parser.add_argument("step", type=str)
    parser.add_argument("device", type=str, default="cuda:0")
    args = parser.parse_args()

    wavlm_model_path = args.wavlm_path
    word2vec_model_path = args.word2vector_path
